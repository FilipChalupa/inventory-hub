...
| `EMAIL_LOCALE`                                                  | no                  | `cs`                      | Language for outbound emails (invitations, reminders, digests): `cs` or `en`. Org-wide — external recipients have no per-user preference.                                             |
| `AUDIT_RETENTION_DAYS`                                          | no                  | –                         | GDPR retention: delete audit-log history older than this many days. Unset keeps it indefinitely.                                                                                      |
